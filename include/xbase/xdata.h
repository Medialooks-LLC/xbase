...
namespace xdata {
     ...
     * @tparam TData The data type to unwrap.
     * @param _data The std::any instance to unwrap.
     * @return The shared_ptr<> to unwrapped data instance or a null pointer if unwrapping failed.
     */
    template <typename TData>
    std::shared_ptr<TData> AnyUnwrapShared(const std::any& _data)
    {
        const auto* data_pp = std::any_cast<std::shared_ptr<TData>>(&_data);
        return data_pp ? *data_pp : nullptr;
    }
     ...
     */
    template <typename TFace>
    const TFace* GetPtr(const IData* _xdata_p, size_t _idx = 0, std::any* _holder_get = nullptr)
    {
        if (!_xdata_p)
            return nullptr;
     ...
     */
    template <typename TFace>
    TFace GetCopy(const IData* _xdata_p, size_t _idx = 0, std::any* _holder_get = nullptr, TFace&& _default = {})
    {
